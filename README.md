...
<!-- tensortonic:start -->
# Inesh Reddy 4015's TensorTonic Solutions

Verified machine learning implementations completed on [TensorTonic](https://www.tensortonic.com).

<p align="center">
  <img src="https://www.tensortonic.com/api/badge/ineshreddy.svg" alt="TensorTonic Verified Solutions" width="100%" />
</p>
...
| Simulate Chunked Prefill Scheduling | Simulate chunked prefill scheduling with strict decode priority, bounded token budgets, and request arrival order. | https://www.tensortonic.com/study-plans/cracking-inference/inference-chunked-prefill-scheduling |
...
| Simulate Disaggregated Prefill and Decode Serving | Simulate Disaggregated Prefill and Decode Serving, and return selected prefill and decode replica indices, TTFT, and completion time for every request. | https://www.tensortonic.com/study-plans/cracking-inference/inference-disaggregated-prefill-decode |
...
| Implement Expert Parallel Routing Across GPUs | Route token-expert assignments to destination GPU ranks with stable ordering, send counts, and an inverse permutation. | https://www.tensortonic.com/study-plans/cracking-inference/inference-expert-parallel-routing |
...
| Implement PagedAttention Block Allocation | Assign each sequence enough fixed-size blocks to hold its tokens, drawn from the free pool in order. | https://www.tensortonic.com/study-plans/cracking-inference/inference-paged-attention-allocation |
| Analyze Prefill and Decode with the Roofline Model | Analyze the prefill and decode phases of transformer inference using the roofline performance model. | https://www.tensortonic.com/study-plans/cracking-inference/inference-prefill-decode-roofline |
| Implement Prefix Cache Matching and Reuse | Find the longest exactly-matching prefix, measured in whole blocks, among all candidates. | https://www.tensortonic.com/study-plans/cracking-inference/inference-prefix-cache-reuse |
...
| Compute TTFT, TPOT, ITL, and Token Throughput | Compute TTFT, TPOT, inter-token latency, and token throughput from request arrival and token timestamp data. | https://www.tensortonic.com/study-plans/cracking-inference/inference-serving-metrics |
| Implement Speculative Decoding and Token Verification | Verify speculative draft tokens against target probabilities and sample the corrected or bonus continuation exactly. | https://www.tensortonic.com/study-plans/cracking-inference/inference-speculative-decoding-verification |
| Calculate Tensor Parallel Communication Cost | Calculate tensor-parallel collective communication volume and latency from message size, world size, bandwidth, and calls. | https://www.tensortonic.com/study-plans/cracking-inference/inference-tensor-parallel-communication |
...
View my verified ML profile: [TensorTonic profile](https://www.tensortonic.com/profile/ineshreddy)
<!-- tensortonic:end -->
